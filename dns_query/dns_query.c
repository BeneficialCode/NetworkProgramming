...
int main(int argc, char* argv[]) {
	...
	// to the end of the query header
	char* p = query + 12; // the query starting at p
	char* h = argv[1]; // use to loop through the hostname

	while (*h) { // *h is equal to zero when we've finished reading the hostname
		char* len = p; // store the position of the label beginning
		p++;
		if (h != argv[1])
			++h;

		// copy characters from *h to *p until we find a dor or the end of the hostname.
		while (*h && *h != '.')
			*p++ = *h++;
		*len = p - len - 1; // sets *len equal to the label length
	}

	// add a terminating 0 byte to finish the name section of the question
	*p++ = 0;
...
const unsigned char* print_name(const unsigned char* msg,
	...
	if ((*p & 0xc0) == 0xc0) {
		// This 14-bit value specifies the location of the name 
		// as an offset from the beginning of the message
...
void print_dns_message(const char* message, int msg_length) {
	// The header is exactly 12 bytes long
	if (msg_length < 12) {
	...
	// the number of records in their corresponding sections
	printf("QDCOUNT = %d\n", qdcount); // the number of questions in a DNS query
	...
	if (qdcount) {
		...
		for (i = 0; i < qdcount; ++i) {
			...
			// the record type we are asking for
	...
	if (ancount || nscount || arcount) {
		...
		for (i = 0; i < ancount + nscount + arcount; ++i) {
			...
			// This field specifies how many seconds the answer is allowed to be cached for.
